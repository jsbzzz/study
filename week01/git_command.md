...
- ### git rebase
  - 현재 브랜치가 파생된 기준점을 대상 브랜치의 최신 커밋으로 재설정한다.  
    ...
    이후 대상 브랜치로 이동해 rebase를 실행한 브랜치를 merge해야 브랜치가 통합된다.
  - 충돌(Conflict) 발생 시 
...
- ### git fetch
  - 원격 저장소의 모든 브랜치의 변경 내역을 로컬 저장소로 가져온다. 기본적인 사용 형태는 다음과 같다.
    ```bash
    git fetch # 기본 원격 저장소(origin)의 모든 변경 내역을 가져온다.
    git fetch --all # 모든 원격 저장소의 변경 내역을 가져온다.
    git fetch --prune # 원격에서 삭제된 브랜치를 로컬에서도 삭제한다.
    git fetch <원격저장소> # 지정한 원격 저장소의 모든 변경 내역을 가져온다.
    git fetch <원격저장소> <브랜치> # 지정한 브랜치의 변경 내역만 가져온다.
    ```
  - fetch를 실행하면 origin/main처럼 <원격저장소>/<브랜치>의 형태로 원격 추적 브랜치를 로컬에 가져온다.  
    원격 추적 브랜치는 직접 수정할 수 없는 읽기 전용 브랜치이고, 이 브랜치로 이동할 시 detached 상태가 된다.
 
- ### git pull
  - 원격 저장소의 변경 내역을 가져와 로컬 브랜치에 merge하는 명령어. 내부적으로는 `git fetch`와 `git merge`가 각각 수행된다.  
    이때 진행되는 병합은 모두 현재 **HEAD**가 위치한 브랜치를 기준으로 한다.
    ```bash
    git pull # 현재 브랜치가 연결된 기본 원격 저장소에서 최신 내역을 가져와 병합한다.
    git pull <원격저장소> # 지정한 원격 저장소에서 현재 브랜치와 연결된 브랜치의 내용을 가져와 병합한다.
    git pull <원격저장소> <브랜치> # 특정 원격 저장소의 특정 브랜치 내용을 가져와 현재 브랜치에 병합한다.
    ```
  - ```bash
    git pull --rebase # rebase 방식으로 변경 내용을 적용한다.
    git pull --ff-only # Fast-Forward 방식으로 병합할 수 있을 때만 실행된다.
    git pull --no-ff # Fast-Forward가 가능해도 무조건 병합커밋을 생성한다.
    ```

- ### git push
  - 로컬에 저장된 커밋 기록을 원격 저장소에 업로드하는 명령어. 사용법은 다음과 같다.
    ```bash
    git push -u <원격저장소> <브랜치> # 대상 브랜치에 커밋 기록을 원격 저장소에 업로드하고 같은 이름의 <원격저장소>/<브랜치>를 upstream으로 정한다.
    git push # 현재 브랜치의 커밋 기록을 브랜치와 연결된 원격 브랜치(upstream)에 업로드한다.
    git push <원격저장소> <브랜치> # 대상 브랜치의 커밋 기록을 같은 이름의 원격 브랜치에 업로드한다.
    git push <원격저장소> <로컬브랜치>:<원격브랜치> # 대상 로컬 브랜치의 커밋 기록을 지정된 원격 브랜치에 업로드한다.
    ```
  - `git push` 명령어의 경우 기본적으로 upstream으로 설정된 원격 브랜치로 push하지만, 이름이 다른 브랜치를 upstream으로 설정할 경우 에러가 발생한다.  
    `git config --global push.default upstream`를 사용하면 `git push`의 기본 목표를 upstream으로 사용할 수 있다.
  - 만약 로컬에서 삭제된 브랜치를 원격에서도 삭제하려면 `git push <원격저장소> --delete <브랜치>` 혹은 `git push <원격저장소> :<브랜치>`를 사용할 수 있다.
  - 추가로 작동 방식은 다음과 같다.
    1. 로컬 브랜치와 원격 저장소의 대응하는 브랜치의 커밋 히스토리를 비교한다.
    2. 로컬에만 존재하는 새로운 커밋들을 서버로 전송한다.
    3. 원격 저장소의 브랜치 포인터를 방금 업로드한 최신 커밋 위치로 갱신한다.
    
  - 만약 로컬 저장소에 없는 커밋이 원격 저장소에 있다면 push가 거절된다.  
    이 경우 `git pull` 명령어로 동기화 후 `git push`하거나 `-f` 옵션을 사용해 강제로 덮어쓸 수 있다.
